...
it is going to be run on windows, using standalone Psychopy3_Py2 (i.e., not the python2.7 version but using python3), from the coder interface.
"""
...
expName = u'word list recall into a microphone'
...
PracticeList1, PracticeList2, LowFrequencyList1, LowFrequencyList2, LowFrequencyList3, LowFrequencyList4, HighFrequencyList1, HighFrequencyList2, HighFrequencyList3, HighFrequencyList4 = define_word_lists()
...
with open(os.path.join(filename, "info.txt"), "w") as f:
    f.write("condition {}\nage {}\ngender {}\nsession {}\ndevmode {}\n".format(condition, expInfo['age'], expInfo['gender'], expInfo['session'], expInfo['devMode']))
...
for blockn in range(5):
    ...
    for listn in range(2):
        ...
        List = block[listn]
        if blockn == 0:
            i = visual.TextStim(win, "please encode each target word in List " + str(listn+1), pos=(0, 0), alignHoriz='center', wrapWidth=1000, height=text_height)
            i.draw()
            win.flip()
            core.wait(i_dur)
            win.flip()
...
# it takes a long time for the audio processes to shut down gracefully, but I don't know why (bad programming from me? natural consequence of how it's done in psychopy?)
